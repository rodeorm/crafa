...
func (s *Server) iterationCreatePost(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logger.Log.Error("IterationCreatePost. levelid",
			zap.Error(err))
		http.Redirect(w, r, "/forbidden", http.StatusTemporaryRedirect)
		return
	}
	...
	if err != nil {
		logger.Log.Error("IterationCreatePost. year",
			zap.Error(err))
		http.Redirect(w, r, "/forbidden", http.StatusTemporaryRedirect)
		return
	}
	...
	if err != nil {
		logger.Log.Error("IterationCreatePost. month",
			zap.Error(err))
		http.Redirect(w, r, "/forbidden", http.StatusTemporaryRedirect)
		return
	}
	...
	if year < 2000 || year > 2050 {
		...
		sign["Err"] = "Год должен быть между 2000 и 2050"
	...
	if month < 1 || month > 12 {
		...
		sign := make(map[string]string)
		sign["Russ"] = "Ошибка при создании итерации"
		sign["Err"] = "Месяц должен быть между 1 и 12"
		at := make(map[string]any)
		at["iter"] = iter
		pg := page.NewPage(page.WithSignals(sign), page.WithAttrs(at), page.WithSession(session))
		page.Execute("iteration", "update", w, pg)
	...
	err = s.stgs.IterationStorager.InsertIteration(context.TODO(), iter)
